...
{
    if (IsSRGB)
        return float4(ConvertToSRGB(Linear.rgb), Linear.a);
    else
        return Linear;
}
...
void main(uint GI : SV_GroupIndex, uint3 DTid : SV_DispatchThreadID)
{
    ...
    if (NonPowerTwo == 0)
    {
        if (SrcMipLevel == 8 && (DTid.x > 1 || DTid.y > 1))
            return;
        
        float2 UV = TexelSize * (DTid.xy + 0.5);
        
        float4 test1 = SrcMip.SampleLevel(PointClamp, float2(0, 0), 5);
        float4 test2 = SrcMip.SampleLevel(PointClamp, float2(0, 0.25), 5);
        float4 test3 = SrcMip.SampleLevel(PointClamp, float2(0.25, 0), 5);
        float4 test4 = SrcMip.SampleLevel(PointClamp, float2(0.25, 0.25), 5);
        
        if (SrcMipLevel == 8)
        {
            Src1 = SrcMip.SampleLevel(BilinearClamp, UV, SrcMipLevel) + (test1 + test2 + test3 + test4) * 0.01f;
        }
        else
        {
    ...
    else if (NonPowerTwo == 1)
    {
        ...
        Src1 = 0.5 * (SrcMip.SampleLevel(BilinearClamp, UV1, SrcMipLevel) +
        SrcMip.SampleLevel(BilinearClamp, UV1 + Off, SrcMipLevel));
    }
    else if (NonPowerTwo == 2)
    {
        // > 2:1 in Y dimension
        // Use 2 bilinear samples to guarantee we don't undersample when downsizing by more than 2x
        // vertically.
    ...
    else if (NonPowerTwo == 3)
    {
        ...
        Src1 = SrcMip.SampleLevel(BilinearClamp, UV1, SrcMipLevel);
