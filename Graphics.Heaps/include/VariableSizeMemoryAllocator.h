...
namespace EduEngine
{
	...
	private:
		...
		using OffsetAlloc = MemoryAllocator::RawMemoryAllocator<std::pair<const size_t, FreeBlockInfo>>;
		typedef std::map<size_t, FreeBlockInfo, std::less<size_t>, OffsetAlloc> TFreeBlocksByOffsetMap;

		using FreeAlloc = MemoryAllocator::RawMemoryAllocator<std::pair<const size_t, TFreeBlocksByOffsetMap::iterator>>;
